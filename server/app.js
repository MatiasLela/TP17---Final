...
app.post('/jugador/agregar', async (req, res) => {
    const {nombre, posicion, edad, estatura, peso, nacionalidad, apariciones, apariciones_sustituto, atajadas, goles_concedidos, asistencias, faltas_cometidas, faltas_sufridas, tarjetas_amarillas, tarjetas_rojas} = req.body;
    const respuesta = await insertarRegistro(nombre, posicion, edad, estatura, peso, nacionalidad, apariciones, apariciones_sustituto, atajadas, goles_concedidos, asistencias, faltas_cometidas, faltas_sufridas, tarjetas_amarillas, tarjetas_rojas);
    res.status(200).send(respuesta);
});

app.put('/jugador/actualizar', async (req, res) => {
    const {id_jugador, nombre, posicion, edad, estatura, peso, nacionalidad, apariciones, apariciones_sustituto, atajadas, goles_concedidos, asistencias, faltas_cometidas, faltas_sufridas, tarjetas_amarillas, tarjetas_rojas} = req.body;
    const respuesta = await actualizarJugadorById(id_jugador, nombre, posicion, edad, estatura, peso, nacionalidad, apariciones, apariciones_sustituto, atajadas, goles_concedidos, asistencias, faltas_cometidas, faltas_sufridas, tarjetas_amarillas, tarjetas_rojas);
    res.status(200).send(respuesta);
});
